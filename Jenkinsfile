        ...
        stage ('Android') {
            ...
            agent {
                node {
                    ...
                    customWorkspace '/home/jenkins/slave-root/dino'
                }
            }
            stages {
                stage ('Clear') {
                    ...
                    stages {                        
                        stage ("Build") {    
                            ...
                            steps {
                                ...
                                withCredentials([usernamePassword(credentialsId: 'UnityUser', usernameVariable: 'UNITY_USER_NAME', passwordVariable: 'UNITY_USER_PASSWORD'), 
                                        string(credentialsId: 'UnityLicenseKey', variable: 'UNITY_LICENSE')]) {                                   
                                    sh 'xvfb-run --auto-servernum --server-args="-screen 0 640x480x24" $UNITY_PATH -batchmode -projectPath . -nographics -quit -serial $UNITY_LICENSE -username $UNITY_USER_NAME -password $UNITY_USER_PASSWORD -logFile -'               
                                }    
                                script {
                                    UNITY_PARAMS=''
                                    UNITY_PARAMS=UNITY_PARAMS + '-loggerLevel ' + params.LoggerLevel
                    ...
                    steps {
                        ...
                        withCredentials([string(credentialsId: 'jenkins_mac_keychain_pass', variable: 'KEYCHAIN_PASSWORD')]) {                      
                            xcodeBuild buildIpa: true, 
                                ...
                                provisioningProfiles: [[provisioningProfileAppId: 'com.feofunlimited.dinoWorldSurvival', provisioningProfileUUID: PROVISION_PROFILE_UUID]], 
